...
def publishParkingSpace(parkingspace_id):
    ...
    try:
        try:
            #start_time,end_time是一个datetime类型的对象
        ...
        new_listing=Parking_time_range(start_time=start_time,end_time=end_time,\
...
def getParkingSpace(parkingspace_id):
    ...
    if all_listings:
        ...
        for each_listing in all_listings:
            duration=(each_listing.end_time-each_listing.start_time).total_seconds()/3600
            listing={"listing_id":each_listing.id,
                "start_time:":each_listing.start_time.strftime('%Y-%m-%d,%H-%M-%S'),
                "end_time":each_listing.end_time.strftime('%Y-%m-%d,%H-%M-%S'),
                "duration":duration,
