...
export default function RentForm({setIsModalVisible, getAllListings}) {
  ...
  const onFinish = async(values) => {
    ...
    console.log('Mydate：',values.date[0].format('YYYY-MM-DD'))
    ...
    fetch("http://127.0.0.1:5000/myrequest/new", requestOption)
    .then(res => {
        if (res.status === 200) {
            return res.json()
        } else {
